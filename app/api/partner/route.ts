...
function userConfirmationHtml(name: string): string {
  return `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>We received your partnership request</title></head>
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;border-radius:24px;overflow:hidden;box-shadow:0 4px 32px rgba(0,0,0,0.10);">
        ...
        <tr>
          <td style="background:linear-gradient(135deg,#0451c4 0%,#056bfa 100%);padding:48px 40px 40px;text-align:center;">
            <div style="display:inline-block;background:rgba(255,255,255,0.15);border-radius:50px;padding:8px 24px;margin-bottom:20px;">
              <span style="color:#ffffff;font-size:13px;font-weight:800;letter-spacing:2px;text-transform:uppercase;">Couponsbit Partnerships</span>
            </div>
            <h1 style="color:#ffffff;font-size:28px;font-weight:900;margin:0 0 8px;line-height:1.3;">Partnership Request Received!</h1>
            <p style="color:rgba(255,255,255,0.85);font-size:15px;margin:0;line-height:1.6;">Thanks for applying to partner with Couponsbit.</p>
          </td>
        </tr>
        ...
        <tr>
          <td style="background:#ffffff;padding:48px 40px;">
            ...
            <p style="color:#555555;font-size:15px;line-height:1.8;margin:0 0 24px;">
              Thank you for your interest in partnering with <strong style="color:#056bfa;">Couponsbit</strong>! We have received your partnership request and our partnerships team will review it shortly.
            </p>
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 32px;">
              <tr>
                <td style="background:linear-gradient(135deg,#f0f6ff 0%,#e8f0fe 100%);border-left:4px solid #056bfa;border-radius:0 12px 12px 0;padding:20px 24px;">
                  <p style="margin:0;color:#0344b0;font-size:14px;font-weight:700;">&#9200; We typically respond within 48 hours</p>
                  <p style="margin:6px 0 0;color:#555555;font-size:13px;">Our partnerships team is available Monday to Saturday, 9 AM – 6 PM IST.</p>
                </td>
              </tr>
            </table>
            ...
            <table width="100%" cellpadding="0" cellspacing="0">
              ${[
                ["&#10003;", "Your partnership request has been safely received"],
                ["&#128269;", "Our partnerships team will review your application"],
                ["&#128231;", "You will receive a detailed reply within 48 hours"],
              ].map(([icon, text]) => `
              <tr>
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="margin:40px 0 0;">
              <tr>
                <td align="center">
                  <a href="https://www.couponsbit.us" style="display:inline-block;background:linear-gradient(135deg,#056bfa,#0451c4);color:#ffffff;font-size:15px;font-weight:800;text-decoration:none;padding:16px 40px;border-radius:50px;letter-spacing:0.5px;">
                    Visit Couponsbit &#8594;
                  </a>
                </td>
              </tr>
        ...
        <tr>
          <td style="background:#ffffff;padding:32px 40px;text-align:center;">
            <p style="color:#999999;font-size:12px;margin:0 0 8px;">You received this email because you submitted the partnership form on Couponsbit.</p>
...
function adminNotificationHtml(data: {
  ...
  return `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>New Partnership Request</title></head>
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;border-radius:24px;overflow:hidden;box-shadow:0 4px 32px rgba(0,0,0,0.12);">
        ...
        <tr>
          <td style="background:#051a33;padding:36px 40px;text-align:center;">
            ...
            <h1 style="color:#ffffff;font-size:24px;font-weight:900;margin:0 0 6px;">New Partnership Request</h1>
            <p style="color:rgba(255,255,255,0.6);font-size:13px;margin:0;">Submitted via Couponsbit Partner Form</p>
          </td>
        </tr>
        ...
        <tr>
          <td style="background:#056bfa;padding:14px 40px;text-align:center;">
            <p style="color:#ffffff;font-size:13px;font-weight:700;margin:0;">&#128276; New request from <strong>${data.fullName}</strong> (${data.companyName})</p>
          </td>
        </tr>
        ...
        <tr>
          <td style="background:#ffffff;padding:40px;">
            ...
            <!-- Message -->
            <p style="color:#111111;font-size:15px;font-weight:800;margin:32px 0 12px;">Business Description</p>
            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="margin:32px 0 0;">
              <tr>
                <td align="center">
                  <a href="mailto:${data.email}?subject=Re: Your partnership request to Couponsbit" style="display:inline-block;background:#056bfa;color:#ffffff;font-size:14px;font-weight:800;text-decoration:none;padding:14px 36px;border-radius:50px;">
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { fullName, email, phone, companyName, partnershipType, message } = body;
    ...
    await Promise.all([
      ...
      transporter.sendMail({
        from: `"Couponsbit Partnerships" <${process.env.GMAIL_USER}>`,
        to: email,
        subject: "We received your partnership request — Couponsbit ✓",
        html: userConfirmationHtml(fullName),
      }),
      // Email to admin
      transporter.sendMail({
        from: `"Couponsbit Partner Form" <${process.env.GMAIL_USER}>`,
        to: process.env.ADMIN_EMAIL,
        subject: `[Partnership] New request from ${fullName} (${companyName}) — Couponsbit`,
        html: adminNotificationHtml({ fullName, email, phone, companyName, partnershipType: partnershipType || "Not specified", message }),
      }),
    ]);
